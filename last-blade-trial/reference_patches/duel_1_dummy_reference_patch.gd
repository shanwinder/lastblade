extends "res://duel_1_dummy_manager.gd"

# =========================
# Duel1DummyReferencePatch.gd
# Step 4: เพิ่ม group fallback ให้ Duel1DummyManager
# ไม่เปลี่ยน logic Duel 1 Guided Training
# =========================
...
func setup_references() -> void:
	...
	# หา Duel1IntroManager: path → group → parent fallback
	duel_intro_manager = get_node_or_null(duel_intro_manager_path)
	if duel_intro_manager == null:
		var intro_nodes := get_tree().get_nodes_in_group("duel_1_intro_manager")
		if intro_nodes.size() > 0:
			duel_intro_manager = intro_nodes[0]
